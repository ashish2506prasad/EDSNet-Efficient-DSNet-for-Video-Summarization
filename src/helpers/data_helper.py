...
class VideoDataset(object):
    ...
    def get_datasets(keys: List[str], where) -> Dict[str, h5py.File]:
        if where == 'kaggle':
            ...
            datasets = {path: h5py.File(os.path.join('/kaggle/input/video-features','/'.join(path.split('/')[2:]) ), 'r') for path in dataset_paths}
            return datasets
        elif where == 'local':
            try:
                dataset_paths = {str(Path(key).parent) for key in keys}
                ...
                dataset_paths = {str(Path(key).parent) for key in keys}
                datasets = {path: h5py.File(os.path.join('DSNet_Kaggle','/'.join(path.split('/')[1:]) ), 'r') for path in dataset_paths}
            return datasets
