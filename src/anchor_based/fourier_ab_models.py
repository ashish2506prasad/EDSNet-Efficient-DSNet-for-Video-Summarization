...
class DSNet_fourier(nn.Module):
    ...
    def forward(self, x):
        _, seq_len, _ = x.shape
        out, temporal_fft_list = self.base_model(x)
        out = out + x
...
class DSNet_fast_fourier(nn.Module):
    def __init__(self, base_model, num_feature, num_hidden, anchor_scales,
                 num_head, fc_depth):
        super().__init__()
        self.anchor_scales = anchor_scales
        self.num_scales = len(anchor_scales)
        self.base_model,temporal_fft_list = build_base_model(base_model, num_feature, num_head)
